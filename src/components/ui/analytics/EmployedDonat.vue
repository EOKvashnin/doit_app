    ...
    >
      ...
      <div class="flex flex-col items-center">
        ...
        <div class="flex items-baseline gap-2">
          ...
          <p class="text-sm font-normal text-gray-500/30 dark:text-gray-300/50">
            {{ firedWorkers }} чел.
            <!-- Исправлено: использовалось workersEmployedPercent вместо firedPercent -->
          </p>
        </div>
      </div>
    </div>
...
const workersEmployedPercent = computed(() => {
  ...
  const rounded = parseInt(percent.toFixed(1))
...
const firedPercent = computed(() => (firedWorkers.value / employedWorkers.value) * 100)
